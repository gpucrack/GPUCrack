...
// Calculates a plaintext from a hash. The result is stored in the plain argument.
...
// Third implementation. No barrel shifting for columnIndex.
void reduceV3(unsigned long int columnIndex, const char* hash, char* plain) {
	for (unsigned long int i = 0; i < PLAIN_LENGTH; i++, plain++, hash++)
		*plain = charset[(unsigned char)(*hash ^ columnIndex) % CHARSET_LENGTH];
}

// Fourth implementation. Incrementation of columnIndex, and + instead of XOR.
...
// Fifth implementation. Incrementation of columnIndex.
